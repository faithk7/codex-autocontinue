...
`install` 只需执行一次：它会将看守进程注册到系统服务管理器（macOS 用 launchd，Linux 用 `systemd --user`，Windows 用任务计划程序），立即启动，把命令加入 PATH，并打印后续步骤。在 macOS 上还会以一连串自动弹出的系统窗口一步完成自动化/辅助功能授权——目标 App 会被隐藏启动，让每个"允许"对话框都在安装期间弹出；辅助功能则通过苹果官方提示窗直接跳到系统设置中唯一需要打开的开关（已预选好）。安装程序会自动验证每一项授权，无需手动在系统设置里逐个寻找开关。程序开机自启，崩溃后自动重启。无需 sudo、brew 或 pip。
