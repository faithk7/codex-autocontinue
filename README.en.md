...
Prerequisite: Python 3 (standard library only, no third-party packages).
...
The helpers — `tmux`, `xdotool` (X11), `ydotool` (Wayland), PowerShell (Windows) — are all optional: when no injector is available on a platform, the tool still detects events and logs "type continue yourself" instead of failing.
...
<details>
<summary>Daemon flags (rarely needed directly)</summary>
...
5. Writes one line per action to `watcher.log`.
...
<details>
<summary>Expand (found during testing; documented so there are no surprises)</summary>
